...
\begin{columns}
    ...
    \block{Application}{
    For \textbf{disk framing}, parallelization is a useful tool to decrease computation time. Disk framing seperated larger-than-RAM datsets into several chunks of data, which are then treated seperately for the task to perform. Afterwords, results are merged again. Through parallelization, chunks of data can be processed parallely and so calculation time is reduced.
     \begin{wrapfigure}{r}{6cm}
     	\includegraphics[width=6cm]{img/disk.framing}
     \end{wrapfigure}
}
    \block{References}{
        \vspace{-1em}
